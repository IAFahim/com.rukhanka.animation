...
namespace Rukhanka
{
    partial struct AnimationProcessSystem
    {
        ...
        {
            [NativeDisableParallelForRestriction] public NativeList<BoneTransform> animatedBonesBuffer;
            [NativeDisableParallelForRestriction] public NativeList<ulong> boneTransformFlagsArr;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            public void Execute(int globalBoneIndex)
            {
                ...
                var (rigBoneIndex, entityIndex, rigBoneCount) = (boneToEntityIndex.y & 0xffff, boneToEntityIndex.x,
                    boneToEntityIndex.y >> 16);
                ...
                var transformFlags = RuntimeAnimationData.GetAnimationTransformFlagsRW(boneToEntityArr,
                    boneTransformFlagsArr, globalBoneIndex, rigBoneCount);
                GetHumanRotationDataForSkeletonBone(out var humanBoneInfo, ref rigBlobAsset.Value.humanData,
                    rigBoneIndex);
                ...
                PrepareRootMotionStateBuffers(e, animationsToProcess, out var curRootMotionState,
                    out var newRootMotionState, rootMotionDeltaBone);
                ...
                for (int ai = 0; ai < animationsToProcess.Length; ++ai)
                {
                    ...
                    var inAvatarMask = IsBoneInAvatarMask(rigBoneIndex, rb.humanBodyPart, atp.avatarMask) ||
                                       rootMotionDeltaBone;
                    if (atp.animation == BlobAssetReference<AnimationClipBlob>.Null || atp.weight == 0 ||
                        atp.layerWeight == 0 || !inAvatarMask)
                        continue;
                    ...
                    {
                        blendedBonePose = BlendLayerPose(blendedBonePose, layerPose, refPose, layerInfo, weightSum,
                            layerFlags);
                        weightSum = 0;
                        layerFlags = 0;
                    ...
                    if (boneHasAnimation)
                    {
                        // === START BOVINELABS TIMELINE OFFSET PATCH ===
                        // Apply offsets exclusively to the root motion bone
                        if (Hint.Unlikely(rootMotionDeltaBone))
                        {
                            if (atp.removeStartOffset)
                            {
                                // FIX: Must use 'boneNameHash' instead of 'rb.hash'. 
                                // If root motion is enabled, the hash is modified. Using rb.hash fails to find the track!
                                SampleAnimation(
                                    ref atp.animation.Value,
                                    0f,
                                    rigBoneIndex,
                                ...
                                bonePose = BoneTransform.Multiply(BoneTransform.Inverse(startPose), bonePose);
                            }

                            var offsetPose = new BoneTransform
                            {
                                pos = atp.positionOffset,
                                rot = atp.rotationOffset,
                                scale = new float3(1, 1, 1)
                            };

                            bonePose = BoneTransform.Multiply(offsetPose, bonePose);

                            // Force TRS flags so Rukhanka applies this to the entity transform
                            flags.x = 1;
                            flags.y = 1;
                        }
                        // === END BOVINELABS TIMELINE OFFSET PATCH ===

                        if (Hint.Unlikely(rootMotionDeltaBone))
                            ProcessRootMotionDeltas(ref bonePose, ref clipTracks, trackRange, atp, curRootMotionState,
                                ref newRootMotionState);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            BoneTransform BlendLayerPose(in BoneTransform curPose, BoneTransform layerPose, in BoneTransform refPose,
                in LayerInfo layerInfo, float weightSum, float3 layerFlags)
            {
                ...
                else
                {
                    ...
                    if (Hint.Likely(layerFlags.y > 0))
                    {
                        quaternion layerRot = math.normalizesafe(new float4(layerPose.rot.value.xyz * layerInfo.weight,
                            layerPose.rot.value.w));
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            )
            {
                ...
                for (;
                     curRootMotionState.IsCreated && historyBufferIndex < curRootMotionState.Length &&
                     curRootMotionState[historyBufferIndex].uniqueMotionId != atp.motionId;
                     ++historyBufferIndex)
                {
                }
                ...
                var rmasc = new RootMotionAnimationStateComponent()
                    { uniqueMotionId = atp.motionId, animationState = bonePose };
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            void SetRootMotionStateToComponentBuffer(in NativeList<RootMotionAnimationStateComponent> newRootMotionData,
                Entity e)
            {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            void GetHumanRotationDataForSkeletonBone(out HumanRotationData rv, ref BlobPtr<HumanData> hd,
                int rigBoneIndex)
            {
                rv = default;
                if (hd.IsValid)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            {
                var r = MuscleRangeToRadians(humanBoneInfo.minMuscleAngles, humanBoneInfo.maxMuscleAngles,
                    bt.rot.value.xyz);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            void CalculateLoopPose(ref TrackSet trackSet, int2 trackRange, ref BoneTransform bonePose,
                in HumanRotationData hrd, float normalizedTime)
            {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            void HandleRootMotionLoops(ref BoneTransform bonePose, ref TrackSet ts, int2 trackRange,
                in AnimationToProcessComponent atp)
            {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            BoneTransform MixPoses(in BoneTransform curPose, BoneTransform inPose, float3 flags, float weight,
                AnimationBlendingMode blendMode)
            {
                ...
                else
                {
                    ...
                    {
                        quaternion layerRot =
                            math.normalizesafe(new float4(inPose.rot.value.xyz * weight, inPose.rot.value.w));
                        layerRot = MathUtils.ShortestRotation(curPose.rot, layerRot);
                        rv.rot = math.mul(curPose.rot, layerRot);
                    }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            public static bool IsBoneInAvatarMask(int boneIndex, AvatarMaskBodyPart humanAvatarMaskBodyPart,
                BlobAssetReference<AvatarMaskBlob> am)
            {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            public static bool IsBoneInHumanAvatarMask(AvatarMaskBodyPart humanBoneAvatarMaskIndex,
                BlobAssetReference<AvatarMaskBlob> am)
            {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            )
            {
                ...
                clipTrackRange = new int2(acb.clipTracks.trackGroups[trackIndex],
                    acb.clipTracks.trackGroups[trackIndex + 1]); // This is safe because of trailing trackGroup
                ...
                if (Hint.Unlikely(isAnimationAdditive))
                {
                    ...
                    ref var additiveTrackSet = ref Hint.Unlikely(acb.additiveReferencePoseFrame.keyframes.Length > 0)
                        ? ref acb.additiveReferencePoseFrame
                        : ref acb.clipTracks;
                    var additiveTrackIndex = additiveTrackSet.trackGroupPHT.Query(boneHash);
                    if (additiveTrackIndex >= 0)
                    {
                        var additivePoseTrackRange = new int2(additiveTrackSet.trackGroups[additiveTrackIndex],
                            additiveTrackSet.trackGroups[additiveTrackIndex + 1]);
                        var additiveFramePose = GetTransformFrame(ref additiveTrackSet, additivePoseTrackRange, hrd,
                            TrackFrame.First);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            (BoneTransform, float3) ProcessAnimationCurves(ref TrackSet ts, int2 trackRange, HumanRotationData hrd,
                float time)
            {
                var rv = BoneTransform.Identity();
...
//=================================================================================================================//
        ...
        {
            void Execute(in DynamicBuffer<AnimationToProcessComponent> animationsToProcess,
                ref DynamicBuffer<AnimatorControllerParameterComponent> acpc)
            {
                ...
                for (var i = 0; i < acpc.Length; ++i)
                {
                    ...
                    p.value.floatValue = ComputeAnimatedProperty(p.value.floatValue,
                        animationsToProcess.AsNativeArray(), SpecialBones.AnimatorTypeNameHash, parameterNameHash);
                }
            }
        }
...
//=================================================================================================================//
        ...
        {
            [ReadOnly] public BufferLookup<AnimationToProcessComponent> animationToProcessLookup;
            [ReadOnly] public ComponentLookup<CullAnimationsTag> cullAnimationsLookup;

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

            void Execute(DynamicBuffer<BlendShapeWeight> blendShapeWeights, in SkinnedMeshRendererComponent asm)
            {
                if (IsAnimationsCulled(asm.animatedRigEntity) ||
                    !animationToProcessLookup.TryGetBuffer(asm.animatedRigEntity, out var animationsToProcess))
                    return;
                ...
                for (var i = 0; i < blendShapeWeights.Length; ++i)
                {
                    ...
                    p.Value = ComputeAnimatedProperty(0, animationsToProcess.AsNativeArray(), asm.nameHash,
                        parameterNameHash);
                }
            }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            {
                return cullAnimationsLookup.HasComponent(animatedEntity) &&
                       cullAnimationsLookup.IsComponentEnabled(animatedEntity);
            }
        }
...
//=================================================================================================================//
        ...
        {
            [ReadOnly] public ComponentTypeHandle<RigDefinitionComponent> rigDefinitionTypeHandle;
            [ReadOnly] public ComponentLookup<CullAnimationsTag> cullAnimationsTagLookup;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            int GetRigBoneCountWithRespectToCulling(Entity e, in RigDefinitionComponent rdc,
                in ComponentLookup<CullAnimationsTag> cullAnimationsTagLookup)
            {
...
//=================================================================================================================//
        ...
        {
            [ReadOnly] public NativeArray<int> chunkBaseEntityIndices;
            [ReadOnly] public NativeList<int2> bonePosesOffsets;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                in v128 chunkEnabledMask)
            {
                ...
                while (cee.NextEntityIndex(out var i))
                {
                    ...
                    var nextOffset =
                        bonePosesOffsets
                            [entityInQueryIndex + 1]; // This is always valid because we have entities count + 1 array
                    ...
                    for (int k = 0; k < boneCount; ++k)
                    {
                        ...
                        boneToEntityIndices[k + offset.x] =
                            new int3(entityInQueryIndex, boneIndexAndBoneCount, offset.y);
                    }
...
//=================================================================================================================//
        ...
        partial struct CopyEntityBoneTransformsToAnimationBuffer : IJobEntity
        {
            ...
            [ReadOnly] public ComponentLookup<RigDefinitionComponent> rigDefComponentLookup;
            [ReadOnly] public ComponentLookup<GPUAnimationEngineTag> gpuAnimationEngineTagLookup;
            [ReadOnly] public ComponentLookup<Parent> parentComponentLookup;
            [ReadOnly] public ComponentLookup<PostTransformMatrix> ptmComponentLookup;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            void Execute(Entity e, in AnimatorEntityRefComponent aer, in LocalTransform lt)
            {
                ...
                var bonePoses = RuntimeAnimationData.GetAnimationDataForRigRW(animatedBoneTransforms,
                    entityBoneData.bonePoseOffset, entityBoneData.rigBoneCount);
                var transformFlags = AnimationTransformFlags.CreateFromBufferRW(boneTransformFlags,
                    entityBoneData.boneFlagsOffset, entityBoneData.rigBoneCount);
...
//=================================================================================================================//
        ...
        {
            [ReadOnly] public ComponentTypeHandle<RigDefinitionComponent> rigDefTypeHandle;
            ...
            [ReadOnly] public NativeList<ulong> boneTransformFlags;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                in v128 chunkEnabledMask)
            {
                ...
                while (cee.NextEntityIndex(out var i))
                {
                    ...
                    var localBoneTransformsForRig =
                        localBoneTransforms.GetSpan(rigFrameData.bonePoseOffset, rigBonesCount);
                    var worldBoneTransformsForRig =
                        worldBoneTransforms.GetSpan(rigFrameData.bonePoseOffset, rigBonesCount);
                    var boneFlags = AnimationTransformFlags.CreateFromBufferRO(boneTransformFlags,
                        rigFrameData.boneFlagsOffset, rigBonesCount);
                    ...
                    for (int animationBoneIndex = 0; animationBoneIndex < rigBonesCount; ++animationBoneIndex)
                    {
                        if (boneFlags.IsAbsoluteTransform(animationBoneIndex))
                        {
                            ...
                            worldBoneTransformsForRig[animationBoneIndex] =
                                localBoneTransformsForRig[animationBoneIndex];
                        }

                        MakeAbsoluteTransform(flagsHolder, animationBoneIndex, localBoneTransformsForRig,
                            worldBoneTransformsForRig, rigDef.rigBlob);
                    }
                    ...
                    for (int animationBoneIndex = 0; animationBoneIndex < rigBonesCount; ++animationBoneIndex)
                    {
                        ...
                        var localTransform = BoneTransform.Multiply(BoneTransform.Inverse(parentWorldTransform),
                            worldTransform);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            )
            {
                ...
                do
                {
                    ...
                    var animatedBoneTransform = absTransformFlag
                        ? worldBoneTransformsForRig[boneIndex]
                        : localBoneTransformsForRig[boneIndex];
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            void Execute(Entity e, in RigDefinitionComponent rdc, ref RootMotionVelocityComponent rmvc,
                in LocalTransform lt)
            {
                ...
                TransformHelpers.ComputeWorldTransformMatrix(e, out var localTransformMatrix, ref localTransformLookup,
                    ref parentLookup, ref ptmLookup);
